...
MyPage::MyPage(string initialPage, map<string, WebPage>* pgMap) {
	...
	outgoingL = new MySearchList(currentOut);
	incomingL = new MySearchList(currentIn);
	...
	outgoingL->updateList(showOut);
	incomingL->updateList(showIn);
...
void MyPage::updateMainP(string mPage) {
	...
	ifstream sourceF;
	cout << "trying to open " << mPage << endl;  //debug
